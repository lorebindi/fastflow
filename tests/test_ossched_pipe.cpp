...
struct Source: ff_node_t<long> {
	...
	int svc_init() {
    	// Imposta l'affinità della CPU per il thread corrente
...
public:
	Manager(ff_pipeline& p, std::chrono::milliseconds i)
		: pipe(p), interval(i), outFile("out.log") {
		if (!outFile.is_open()) {
...
/*void manager(ff_pipeline& pipe, chrono::milliseconds interval) {
...
int main(int argc, char* argv[]) {
    ...
    size_t ntasks = 10000;
    size_t nnodes = 2;
	int interval_ms = 1000; // Intervallo di default (1 secondo)
	...
	if (argc > 1) {
		...
		interval_ms = std::stoi(argv[3]); // Leggi l'intervallo in millisecondi
	}

	// Definisci un set di CPU
	cpu_set_t cpuset;
	CPU_ZERO(&cpuset); // Azzera il set di CPU
	CPU_SET(0, &cpuset);
	CPU_SET(16, &cpuset);
	//CPU_SET(32, &cpuset);
	//CPU_SET(48, &cpuset);
	...
	std::thread managerThread([&manager]() { manager.start(); });
